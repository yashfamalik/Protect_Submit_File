...
// Helper function to load setup data from localStorage
const loadSetupDataFromLocalStorage = () => {
  try {
    const stored = localStorage.getItem('setupData');
    if (stored) {
      return JSON.parse(stored);
    }
  } catch (error) {
    console.warn('Failed to load setup data from localStorage:', error);
  }
  return null;
};
...
export default function mountSetup(container, props = {}) {
  ...
  const loadedFromStorage = loadSetupDataFromLocalStorage();

  // State
  const state = {
    selectedEligibleId: loadedFromStorage?.state?.selectedEligibleId || null,
    quantities: loadedFromStorage?.state?.quantities || {},
    selectedAction: loadedFromStorage?.state?.selectedAction || "reorder",
    selectedClaimOption: loadedFromStorage?.state?.selectedClaimOption || "Product damaged", // Default to first option
    claimDetails: loadedFromStorage?.state?.claimDetails || "",
    supportingFiles: [], // Cannot restore files from localStorage
    submitting: false,
  ...
  const proxy = {
    ...
    saveToLocalStorage() {
      try {
        const data = {
          ...
          colorSettings: this.colorSettings,
          state: {
            selectedEligibleId: state.selectedEligibleId,
            quantities: { ...state.quantities },
            selectedAction: state.selectedAction,
            selectedClaimOption: state.selectedClaimOption,
            claimDetails: state.claimDetails,
            supportingFiles: state.supportingFiles.map(file => ({
              name: file.name,
              size: file.size,
              type: file.type
        ...
        console.warn('Failed to save setup data to localStorage:', error);
      }
    },
    ...
    loadFromLocalStorage() {
      try {
        ...
        if (stored) {
          ...
          this.contentSettings = data.contentSettings || null;
          this.colorSettings = data.colorSettings || null;
          
          // Restore state if available
          if (data.state) {
            if (data.state.selectedEligibleId !== undefined) {
              state.selectedEligibleId = data.state.selectedEligibleId;
            }
            if (data.state.quantities) {
              Object.assign(state.quantities, data.state.quantities);
            }
            if (data.state.selectedAction) {
              state.selectedAction = data.state.selectedAction;
            }
            if (data.state.selectedClaimOption) {
              state.selectedClaimOption = data.state.selectedClaimOption;
            }
            if (data.state.claimDetails) {
              state.claimDetails = data.state.claimDetails;
            }
            // Note: supportingFiles cannot be restored from localStorage due to security restrictions
          }
          return true;
        }
      } catch (error) {
    ...
    clearLocalStorage() {
      try {
        ...
        console.warn('Failed to clear setup data from localStorage:', error);
      }
    },
  ...
  if (typeof window !== 'undefined' && window.SetupProxy) {
    ...
    // Load data from localStorage even if no SetupProxy
    proxy.loadFromLocalStorage();
  ...
  function rerender() {
    // Save state to localStorage before re-rendering
